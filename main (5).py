...
import sqlite3
import requests
import re  # Added for amount detection regex
...
HISTORY_CHANNEL_CHAT_ID = "@ZyroHistory"  
GIFTS_CHANNEL_CHAT_ID = "@ZyroGifts"  # New channel for gift logs
...
def init_db():
    ...
    run_query("CREATE TABLE IF NOT EXISTS users (user_id INTEGER PRIMARY KEY, username TEXT, balance REAL DEFAULT 0.0, total_deposited REAL DEFAULT 0.0, total_spent REAL DEFAULT 0.0, status TEXT DEFAULT 'active')")
    ...
    run_query("CREATE TABLE IF NOT EXISTS history (id INTEGER PRIMARY KEY AUTOINCREMENT, user_id INTEGER, service TEXT, number TEXT, otp TEXT, server TEXT, cost REAL DEFAULT 0.0, timestamp DATETIME DEFAULT CURRENT_TIMESTAMP)")
    
    gift_cfg = run_query("SELECT value FROM sys_config WHERE key = 'gift_system'", is_select=True, fetch_all=False)
    if not gift_cfg:
        run_query("INSERT INTO sys_config (key, value) VALUES ('gift_system', 'on')")
...
def ensure_user(user_id, username=None):
    ...
    if not res: 
        run_query("INSERT INTO users (user_id, username, balance, total_deposited, total_spent, status) VALUES (?, ?, 0.0, 0.0, 0.0, 'active')", (user_id, username))
    elif username:
        run_query("UPDATE users SET username = ? WHERE user_id = ?", (username, user_id))

def get_typing_area_keyboard(uid, uname=None):
    markup = ReplyKeyboardMarkup(resize_keyboard=True, row_width=2)
    
    gift_cfg = run_query("SELECT value FROM sys_config WHERE key = 'gift_system'", is_select=True, fetch_all=False)
    gift_on = gift_cfg[0] == 'on' if gift_cfg else True
    
    # 🌟 Fixed exact requested button layout structure 🌟
    if gift_on:
        markup.add(KeyboardButton("Check Service"), KeyboardButton("Balance"))
        markup.add(KeyboardButton("Support"), KeyboardButton("Send Gift"))
    else:
        markup.add(KeyboardButton("Check Service"), KeyboardButton("Balance"))
        markup.add(KeyboardButton("Support"))
        
    if is_admin(uid, uname):
        markup.add(KeyboardButton("Advance Control"))
        
    return markup

def send_force_join_msg(chat_id):
    text = "💪🏻 𝐖e𝐥𝐜𝐨𝐦e 𝐭𝐨 𝐙𝐲𝐫𝐨𝐒𝐌𝐒\n\n━━━━━━━━━━━━━━━━━━━━━━\n\n⚠️ *Aapne hamara update channel join nahi kiya hai!*\n\nBot use karne ke liye pehle niche click karke join karein aur Verify par click karein.\n\n💬 Telegram = @ZyroSMS\n\n━━━━━━━━━━━━━━━━━━━━━━"
...
@bot.message_handler(func=lambda message: message.text in ["Check Service", "Balance", "Support", "Send Gift", "Advance Control"])
def handle_typing_area_buttons(message):
    uid = message.chat.id
    uname = message.from_user.username
    if not check_status(uid, uid): return
    
    if message.text == "Check Service":
        ...
        support_text = (
            "🛎️ 𝐂𝐮𝐬𝐭𝐨𝐦𝐞𝐫 𝐒𝐮𝐩𝐩𝐨𝐫𝐭\n"
            "━━━━━━━━━━━━━━━━━━━━━━\n\n"
            "Welcome to ZyroSMS Support! 💙\n\n"
            "👤 𝐀𝐝𝐦𝐢𝐧\n"
            "➜ @ZyroSMS\n\n"
            "📢 𝐁𝐨𝐭 𝐔𝐩𝐝𝐚𝐭𝐞𝐬\n"
            "➜ https://t.me/ZyroUpdate\n\n"
        ...
        bot.send_message(uid, support_text, disable_web_page_preview=True)
    bot.send_message(uid, support_text, disable_web_page_preview=True)
    elif message.text == "Send Gift":
        gift_cfg = run_query("SELECT value FROM sys_config WHERE key = 'gift_system'", is_select=True, fetch_all=False)
        if gift_cfg and gift_cfg[0] == 'off':
            return
        gift_start_process(message)
    elif message.text == "Advance Control":
        if is_admin(uid, uname):
            advanced_admin_hub(message)

def send_main_menu_direct(uid, uname=None):
    text = "💪🏻 𝐖e𝐥𝐜𝐨𝐦e 𝐭𝐨 𝐙𝐲𝐫𝐨𝐒𝐌𝐒\n\n━━━━━━━━━━━━━━━━━━━━━━\n\n🤝 𝗔𝗹𝗹 𝗧𝘆𝗽𝗲 𝗢𝗧𝗣 𝗔𝘃𝗮𝗶𝗹𝗮б𝗹𝗲 𝗶𝗻 𝗖𝗵𝗲𝗮𝗽 𝗣𝗿𝗶𝗰су\n\n💬 Contact Telegram = @ZyroSMS\n\n━━━━━━━━━━━━━━━━━━━━━━\n 𝐓𝐡𝐚𝐧𝐤 𝐲𝐨𝐮 𝐟𝐨𝐫 𝐜𝐡𝐨𝐨𝐬𝐢𝐧𝐠 𝐙𝐲𝐫𝐨𝐒𝐌𝐒 !:"
    bot.send_message(uid, text, reply_markup=get_typing_area_keyboard(uid, uname))
...
# =====================================================================
# 🎁 SEND GIFT FEATURE SYSTEM
# =====================================================================
def gift_start_process(message):
    text = (
        "🎁 𝐒𝐞𝐧𝐝 𝐆𝐢𝐟𝐭\n"
        "━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "𝗦𝗵𝗮𝗿𝗲 𝘆𝗼𝘂𝗿 𝗯𝗮𝗹𝗮𝗻𝗰𝗲 𝘄𝗶𝘁𝗵 𝘆𝗼𝘂𝗿 𝗳𝗿𝗶𝗲𝗻𝗱𝘀 𝗶𝗻𝘀𝘁𝗮𝗻𝘁𝗹𝘆 !  💙\n\n"
        "👤 𝐏𝐥𝐞𝐚𝐬𝐞 𝐞𝐧𝐭𝐞𝐫 𝐭𝐡𝐞 𝐅𝐫𝐢𝐞𝐧𝐝 𝐔𝐬𝐞𝐫𝐧𝐚𝐦𝐞.\n\n"
        "𝗘𝘅𝗮𝗺𝗽𝗹𝗲 :\n"
        "@AwaisFS"
    )
    msg = bot.send_message(message.chat.id, text)
    bot.register_next_step_handler(msg, gift_collect_username)

def gift_collect_username(message):
    friend_uname = message.text.strip().replace("@", "")
    
    target = run_query("SELECT user_id FROM users WHERE LOWER(username) = LOWER(?)", (friend_uname,), is_select=True, fetch_all=False)
    if not target:
        bot.send_message(message.chat.id, "❌ User database tracking entry matched record not found. Ask friend to at least start the bot once.")
        return
        
    target_uid = target[0]
    
    if target_uid == message.chat.id:
        bot.send_message(message.chat.id, "❌ You cannot gift balance to yourself.")
        return

    text = (
        "💵 𝐆𝐢𝐟𝐭 𝐀𝐦𝐨𝐮𝐧𝐭\n"
        "━━━━━━━━━━━━━━━━━━━━━━\n\n"
        "𝐄𝐧𝐭𝐞𝐫 𝐭𝐡𝐞 𝐀𝐦𝐨𝐮𝐧𝐭 𝐲𝐨𝐮 𝐰𝐚𝐧𝐭 𝐭𝐨 𝐒𝐞𝐧𝐝.\n\n"
        "💡 𝗘𝘅𝗮𝗺𝗽𝗹𝗲 : 0.05,0.5,1,2"
    )
    msg = bot.send_message(message.chat.id, text)
    bot.register_next_step_handler(msg, gift_collect_amount, target_uid, friend_uname)

def gift_collect_amount(message, target_uid, friend_uname):
    sender_uid = message.chat.id
    sender_uname = message.from_user.username or "No_Username"
    raw_amount = message.text.strip()
    
    match = re.search(r"⏳?\d+(\.\d+)?", raw_amount)
    if not match:
        bot.send_message(sender_uid, "❌ Amount validation entity missing layout parser error.")
        return
        
    try:
        amount = float(match.group())
    except ValueError:
        bot.send_message(sender_uid, "❌ Invalid numeric processing amount structure format.")
        return
        
    if amount <= 0:
        bot.send_message(sender_uid, "❌ Gift amount must be positive real entity validation constraint.")
        return
        
    sender_bal_res = run_query("SELECT balance FROM users WHERE user_id = ?", (sender_uid,), is_select=True, fetch_all=False)
    sender_bal = sender_bal_res[0] if sender_bal_res else 0.0
    
    if sender_bal < amount and not is_admin(sender_uid, message.from_user.username):
        bot.send_message(sender_uid, f"❌ Inefficient logic matching constraints, Current balance: ${sender_bal:.2f}")
        return

    animation_msg = bot.send_message(sender_uid, "⏳ 𝐏𝐫𝐨𝐜𝐞𝐬𝐬𝐢𝐧𝐠...")
    frames = ["⏳ 𝐏𝐫𝐨𝐜𝐞𝐬𝐬𝐢𝐧𝐠...", "⏳ 𝐏𝐫𝐨𝐜𝐞𝐬𝐬𝐢𝐧𝐠.. 💸", "⏳ 𝐏𝐫𝐨𝐜𝐞𝐬𝐬𝐢𝐧𝐠... 🚀", "⏳ 𝐏𝐫𝐨𝐜𝐞𝐬𝐬𝐢𝐧𝐠. 💙", "⏳ 𝐏𝐫𝐨𝐜𝐞𝐬𝐬𝐢𝐧𝐠... ✨"]
    for i in range(4):
        time.sleep(1.25)
        try: bot.edit_message_text(frames[i+1], sender_uid, animation_msg.message_id)
        except Exception: pass
        
    if not is_admin(sender_uid, message.from_user.username):
        run_query("UPDATE users SET balance = MAX(0.0, balance - ?) WHERE user_id = ?", (amount, sender_uid))
    run_query("UPDATE users SET balance = balance + ? WHERE user_id = ?", (amount, target_uid))
    ...
    sender_receipt = (
        ...
        f"👤 𝐅𝐫𝐢𝐞𝐧𝐝 \n➜ @{friend_uname}\n\n"
        f"💵 𝐀𝐦𝐨𝐮𝐧𝐭\n➜ ${amount:.2f}\n\n"
        "✅ 𝐘𝐨𝐮𝐫 𝐆𝐢𝐟𝐭 𝐡𝐚𝐬 𝐛𝐞𝐞𝐧 𝐬𝐞𝐧𝐭 𝐒𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲."
    )
    bot.send_message(sender_uid, sender_receipt)
    
    # Keeping the layout structure updated dynamically with persistent main buttons
    receiver_markup = get_typing_area_keyboard(target_uid)
    ...
    receiver_receipt = (
        ...
        f"👤 𝐅𝐫𝐨𝐦\n➜ @{sender_uname}\n\n"
        f"💵 𝐀𝐦𝐨𝐮𝐧𝐭\n➜ ${amount:.2f}\n\n"
        "💙 𝐓𝐡𝐞 𝐆𝐢𝐟𝐭 𝐡𝐚𝐬 𝐛𝐞𝐞𝐧 𝐚𝐝ｄeadded 𝐭𝐨 𝐲𝐨𝐮𝐫 𝐁𝐚𝐥𝐚𝐧𝐜𝐞 𝐒𝐮𝐜𝐜𝐞𝐬𝐬𝐟𝐮𝐥𝐥𝐲.\n"
        "━━━━━━━━━━━━━━━━━━━━━━"
    )
    bot.send_message(target_uid, receiver_receipt, reply_markup=receiver_markup)
    ...
    try: bot.send_message(GIFTS_CHANNEL_CHAT_ID, group_broadcast)
    except Exception as e: logger.error(f"Gifts Log channel error trace: {e}")

# =====================================================================
# 👑 /ADMIN ADVANCED CONTROL PLATFORM INTERFACE
# =====================================================================
...
def advanced_admin_hub(message):
    ...
    gift_cfg = run_query("SELECT value FROM sys_config WHERE key = 'gift_system'", is_select=True, fetch_all=False)
    status_gift = "OFF 🛑" if gift_cfg and gift_cfg[0] == 'off' else "ON 🟢"
    
    text = "💪🏻 *𝐀𝐝𝐯𝐚𝐧𝐜𝐞 𝐂𝐨𝐧𝐭𝐫𝐨λ 𝐇𝐮𝐛*\n━━━━━━━━━━━━━━━━━━━━━━"
    markup = InlineKeyboardMarkup(row_width=1)
    markup.add(
        ...
        InlineKeyboardButton("🙈 Hide Server (S1/S2)", callback_data="adm_panel_hidesrv"), 
        InlineKeyboardButton(f"Gift System: {status_gift}", callback_data="adm_panel_togglegift"),
...
def handle_admin_panel_clicks(call):
    ...
    elif action == "togglegift":
        cfg = run_query("SELECT value FROM sys_config WHERE key = 'gift_system'", is_select=True, fetch_all=False)
        curr = cfg[0] if cfg else "on"
        new_val = "off" if curr == "on" else "on"
        run_query("UPDATE sys_config SET value = ? WHERE key = 'gift_system'", (new_val,))
        bot.send_message(uid, f"⚙️ Gift Transfer System feature sets switched to: *{new_val.upper()}*")
    elif action == "hidesrv":
    ...
    elif action == "uhistory":
        summary_query = (
            "SELECT u.username, h.service, COUNT(h.id) FROM history h "
            "JOIN users u ON h.user_id = u.user_id "
            "GROUP BY u.username, h.service ORDER BY u.username"
        )
        hist = run_query(summary_query, is_select=True)
        if not hist:
            bot.send_message(uid, "No log records found in history table database tracker.")
            return
            
        text = "📋 *Services Mapped OTP Log Summary Overview:*\n\n"
        for item in hist:
            uname_disp = f"@{item[0]}" if item[0] else "Unknown_User"
            srv = item[1]
            if "douyin" in srv.lower(): srv = "Dy"
            elif "rednote" in srv.lower(): srv = "Red"
            elif "whatsapp" in srv.lower(): srv = "Wa"
            elif "bilibili" in srv.lower(): srv = "Bili"
            elif "telegram" in srv.lower(): srv = "Teli"
            
            text += f"{uname_disp} = {srv} = {item[2]} Otp\n"
        bot.send_message(uid, text)
        
    elif action == "phistory":
        msg = bot.send_message(uid, "⌨️ Enter the Target Friend Username to lookup wallet statistics list detail rows:")
        bot.register_next_step_handler(msg, admin_lookup_pay_history_by_username)

def admin_lookup_pay_history_by_username(message):
    if not is_admin(message.from_user.id, message.from_user.username): return
    target_uname = message.text.strip().replace("@", "")
    
    usr_data = run_query("SELECT user_id, balance FROM users WHERE LOWER(username) = LOWER(?)", (target_uname,), is_select=True, fetch_all=False)
    if not usr_data:
        bot.send_message(message.chat.id, "❌ User entry profile records logic missing in current ledger configurations.")
        return
        
    t_uid, t_bal = usr_data[0], usr_data[1]
    
    usage_query = "SELECT service, COUNT(id), SUM(cost) FROM history WHERE user_id = ? GROUP BY service"
    usage_records = run_query(usage_query, (t_uid,), is_select=True)
    
    report = (
        f"💳 *Wallet Ledger Account Details Summary*\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"👤 User: @{target_uname}\n"
        f"💵 Total Balance = ${t_bal:.2f}\n"
        f"━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📊 *Balance use metrics trace lines:*\n"
    )
    
    if not usage_records:
        report += "\n_No usage records traced for this user entity down the database line._"
    else:
        for row in usage_records:
            report += f"➜ Service = {row[0]} | Otp = {row[1]} | Spent = ${row[2] or 0.0:.2f}\n"
...
# =====================================================================
# Realtime dynamic service settings operational loops
# =====================================================================
...
def run_s1(call):
    ...
    try: _, _, _, app_code, country_code = call.data.split("_")
    except Exception as e: logger.error(f"Callback split mismatch: {e}"); return
    ...
    if not price_res: bot.send_message(user_id, error_text); return
    ...
    if not is_admin(user_id, call.from_user.username):
        ...
        if balance < cost:
            bot.send_message(user_id, f"♎  𝐋𝐨𝐰 𝐰𝐚𝐥𝐥𝐞𝐭 𝐛𝐚𝐥𝐚𝐧𝐜𝐞! 𝐏𝐫𝐢𝐜𝐞 𝐫𝐞𝐪𝐮𝐢𝐫𝐞ｄ: ${cost:.2f}")
            return
        
    api_res = run_query("SELECT api_code, name FROM services WHERE code = ?", (app_code,), is_select=True, fetch_all=False)
    if not api_res: bot.send_message(user_id, error_text); return
    ...
    try:
        ...
        logger.error(f"API Conn error: {e}"); bot.send_message(user_id, error_text)
    ...
    while time.time() - start_time < 1200:
        ...
        try:
            ...
            if res_text.startswith("STATUS_OK"):
                ...
                run_query("INSERT INTO history (user_id, service, number, otp, server, cost) VALUES (?, ?, ?, ?, 'Server 1', ?)", (user_id, app_name, phone_number, otp_code, cost))
                text = f"💪🏻 𝐎𝐓𝐏 𝐑e𝐜e𝐢𝐯e𝐝\n\nService : {app_name}\n━━━━━━━━━━━━━━━━━━━━━━\n🧾 𝐍𝐮𝐦𝐛𝐞𝐫\n\n`+{phone_number}`\n\n━━━━━━━━━━━━━━━━━━━━━━\n🔐 OTP\n\n`{otp_code}`"
                markup = InlineKeyboardMarkup(row_width=1)
                markup.add(
...
def cancel_s1_(call):
    ...
    if time.time() - order["timestamp"] < 120:
...
def run_s2(call):
    ...
    try: _, _, _, app_code, country_code = call.data.split("_")
    except Exception as e: logger.error(f"Callback split mismatch S2: {e}"); return
...
def admin_sends_s2_otp(message, req_id):
    ...
    run_query("INSERT INTO history (user_id, service, number, otp, server, cost) VALUES (?, ?, ?, ?, 'Server 2', ?)", (ord_data["user_id"], ord_data["app_name"], ord_data["number"], otp_val, cost))
...
# =====================================================================
# 💳 ADVANCED DYNAMIC USER PAYMENT ENGINE
# =====================================================================
...
def pay_gate_init(call):
    ...
    if gateway == "easypaisa":
        text = "<b>𝐀𝐜𝐜𝐨𝐮𝐧𝐭 𝐃𝐞𝐭𝐚𝐢𝐥𝐬 </b> 📃 \n\n<b>𝗔𝗰𝗰𝗼𝘂𝗻𝘁 :</b> Easypaisa\n<b><b>𝗡𝗮𝗺𝗲 :</b></b> Saira Bibi\n<b><b>𝗡𝘂𝗺𝗯𝗲𝗿 :</b></b> <code>03087886967</code>\n\n⚠️ <i><b>𝐏λ𝐞𝐚𝐬𝐞 𝐭𝐚𝐤𝐞 𝐚 𝐒𝐜𝐫𝐞𝐞𝐧𝐬𝐡𝐨𝐭 𝐚𝐟𝐭𝐞𝐫 𝐦𝐚𝐤𝐢𝐧𝐠 𝐭𝐡𝐞 𝐏𝐚𝐲𝐦𝐞𝐧𝐭:</b></i>"
    elif gateway == "jazzcash":
        text = "<b>𝐀𝐜𝐜𝐨𝐮𝐧𝐭 𝐃𝐞𝐭𝐚𝐢𝐥𝐬 </b> 📃 \n\n<b>𝗔𝗰𝗰𝗼𝘂𝗻𝘁 :</b> Jazzcash\n<b><b>𝗡𝗮𝗺𝗲 :</b></b> Saira Bibi\n<b><b>𝗡𝘂𝗺𝗯𝗲𝗿 :</b></b> <code>03087886967</code>\n\n⚠️ <i><b>𝐏𝐥𝐞𝐚𝐬𝐞 𝐭𝐚𝐤𝐞 𝐚 𝐒𝐜𝐫𝐞𝐞𝐧𝐬𝐡𝐨𝐭 𝐚𝐟𝐭𝐞𝐫 𝐦𝐚𝐤𝐢𝐧𝐠 𝐭𝐡𝐞 𝐏𝐚𝐲𝐦𝐞𝐧𝐭:</b></i>"
    elif gateway == "binance":
        text = "<b>𝐀𝐜𝐜𝐨𝐮𝐧𝐭 𝐃𝐞𝐭𝐚𝐢𝐥𝐬 </b> 📃 \n\n<b>𝗔𝗰𝗰𝗼𝘂𝗻𝘁 :</b> Binance Pay\n<b>𝗨𝘀𝗲𝗿:</b> <code>Zayan_2662</code>\n<b>𝗕𝗶𝗻𝗮𝗻𝗰𝗲 𝗜𝗗:</b> <code>1178891914</code>\n\n⚠️ <i><b>𝐏𝐥𝐞𝐚𝐬𝐞 𝐭𝐚𝐤𝐞 𝐚 𝐒𝐜𝐫𝐞𝐞𝐧𝐬𝐡𝐨𝐭 𝐚𝐟𝐭𝐞𝐫 𝐦𝐚𝐤𝐢𝐧𝐠 𝐭𝐡𝐞 𝐏𝐚𝐲𝐦𝐞𝐧𝐭:</b></i>"
    else: text = "Invalid gateway."
...
def collect_payment_screenshot(message, gateway):
    ...
    bot.send_message(uid, "⏳ 𝐘𝐨𝐮𝐫 𝐩𝐚𝐲𝐦𝐞𝐧𝐭 𝐢𝐬 𝐮𝐧𝐝𝐞𝐫 𝐫e𝐯𝐢𝐞𝐰. 𝐏𝐥e𝐚𝐬e 𝐰𝐚𝐢𝐭.")
    ...
    try: bot.send_photo(PAYMENT_GROUP_CHAT_ID, file_id, caption=grp_text, reply_markup=markup, parse_mode="HTML")
    except Exception as e:
        logger.error(f"Group forward error: {e}")
        bot.send_message(uid, "⚠️ System error: Forwarding failed.")
...
def group_payment_verdict(call):
    ...
    if ticket_id not in payment_temp_cache: return
...
def group_finalizes_credit_load(message, ticket_id):
    ...
    except Exception: is_group_admin = False

    if not is_group_admin: return
    ...
    try:
        ...
        msg = bot.send_message(PAYMENT_GROUP_CHAT_ID, "❌ Parsing numeric entity failure. Send exact numeric value:")
        bot.register_next_step_handler(msg, group_finalizes_credit_load, ticket_id)
